...
####################### Paper
# Word Corpus as a Feature Vector for Journals
...
In this paper a workflow for document similarity comparisons is explained. Many text
...
each subgroup was compared.


## Introduction

Document comparison can be performed in many ways. One way is to construct word
...
PubMed abstract articles are loaded into a distributed computing
...
There are many techniques to extract information from a body of text.
...
it assume that variance is homogenous, unlike ANOVA.

It is important to understand the confidence interval and p-value when rejecting the null or
...
WRS2 package used in the analysis below.
...
```{r}
workingDir <- "C:/Users/seand/OneDrive/Documents/Class_Docs/RBDA_project_data/diabetes_asthma_hiv/"
dir(workingDir)
...
```{r eval=FALSE}
asthma$journal_asthma %>% n_distinct() # 5

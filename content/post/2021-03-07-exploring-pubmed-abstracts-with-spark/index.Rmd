...
val joinedDf = journalDense.join(diabetes_journalDense, 'journalTitle =!= 'journalTitle_subset)
val cosSimDf = joinedDf.withColumn("cosine_sim", cosSimilarity('features_subset, 'features)).select('journalTitle, 'journalTitle_subset, 'cosine_sim)
```
...
These are the top percentiles of cosines for each comparison group. I selected the top 97.5%-ile for Diabetes and HIV, while lowering the percentile to 75% for the Asthma group to get a similar length output.
...
diabetes$journal_x[diabetes$cosine_similarity > quantile(ingroup_diabetes_cossim, probs = 0.975) & !(diabetes$journal_x %in% diabetes$journal_diabetes)]
```

```{r}
ingroup_hiv_cossim <- hiv$cosine_similarity[hiv$journal_x %in% hiv$journal_hiv]
hiv$journal_x[hiv$cosine_similarity > quantile(ingroup_hiv_cossim, probs = 0.975) & !(hiv$journal_x %in% hiv$journal_hiv)]
```
...
hist(asthma$X3, breaks = 100, main = "Similarity To 'HIV' Subset", xlab = "Cosine Similarity")
hist(hiv$X3, breaks = 100, main = "Similarity To 'Asthma' Subset", xlab = "Cosine Similarity")
```
...
![](static/images/post/2021-03-07-exploring-pubmed-abstracts-with-spark/QuantDiffDiabetesAsthma.jpg)

![](static/images/post/2021-03-07-exploring-pubmed-abstracts-with-spark/QuantDiffDiabetesHIV.jpg)

![](static/images/post/2021-03-07-exploring-pubmed-abstracts-with-spark/quantDiffAsthmaHIV.jpg)
